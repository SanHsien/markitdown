...
歡迎能保住 Windows 相容性、轉換結構完整度與低維護成本的小改動。

## 開始前

1. 先讀 [`AGENTS.md`](AGENTS.md)、[`FORK.md`](FORK.md) 與 [`README.md`](README.md)。
2. 確認問題在最新 `main` 仍可重現，並查過既有 Issues。
3. 產品核心行為的實質變更，優先考慮回報或回貢 [`microsoft/markitdown`](https://github.com/microsoft/markitdown)。
...
這只驗證維護骨架。套件產品測試與擴充格式安裝見 [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)。
...
- 說明是否來自 upstream、是否改動 `packages/` 內容，以及實際跑過哪些指令。
