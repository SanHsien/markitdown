...
`tools/check_dependency_freshness.py` 比對 `requirements-dev.txt`、`requirements.txt` 與 `requirements-all.txt` 的宣告。產品套件亦配置 Dependabot 追蹤更新。

紅燈只有兩條誠實的出口：

| 出口 | 寫在哪 | 什麼時候用 |
| --- | --- | --- |
| `# freshness-hold: <理由>` | requirements 檔案行末 | 這個下限就是我們要的 |
